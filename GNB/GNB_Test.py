...
class GaussianNBTester:
    def __init__(self, model_path='./GNB/model/gaussian_nb_model.pkl', feature_path='./GNB/model/model_features.pkl'):
        """
        初始化测试器，加载模型和特征信息
        """
        try:
            ...
            print(f"模型加载成功！类别标签: {self.feature_info['class_labels']}")
...
# 编程接口函数
def load_GNB(model_path='./GNB/model/gaussian_nb_model.pkl', feature_path='./GNB/model/model_features.pkl'):
    """
    加载训练好的高斯朴素贝叶斯模型
    Returns:
        GaussianNBTester实例
    """
    return GaussianNBTester(model_path, feature_path)

def predict_single(model_tester, test_cases):
    """
    对单个测试用例进行预测的接口
    Args:
        model_tester: GaussianNBTester实例
        test_cases: 测试用例列表
    """
...
def GNB_Test(test_df):
    ...
    if GNB_model.model is not None:
        try:
            # 尝试加载测试数据
            batch_result = predict_batch(GNB_model, test_df)
...
def GNB_Prediction(test_cases):
    ...
    if GNB_model.model is not None:
        predictions, probabilities = predict_single(GNB_model, test_cases)
        return predictions, probabilities
